module.exports = async function handler(req, res) {
  ...
  if (callbackQuery) {
    ...
    await saveClient({
      ...
      contact:username,
      tariff:selected,
      request:`Користувач обрав формат: ${selected}`,
      note:"",
      source:"telegram",
      status:"new",
  ...
  if (String(userChatId) === adminChatId && text.startsWith("/reply ")) {
    ...
    if (!target || !replyText) {
      await sendMessage(adminChatId, "Невірний формат. Використовуйте: /reply CHAT_ID текст відповіді");
      return res.status(200).json({ok:true});
    }
  ...
  if (text === "/consultation") {
    await sendMessage(userChatId, "Оберіть формат консультації:", { reply_markup: mainKeyboard() });
    return res.status(200).json({ok:true});
  }

  if (text === "/help") {
    await sendMessage(userChatId, "/start — меню\n/consultation — обрати консультацію");
    return res.status(200).json({ok:true});
  }

  const hasPhoto = Array.isArray(message.photo) && message.photo.length > 0;
  const hasDocument = !!message.document;
  const hasReceiptFile = hasPhoto || hasDocument;

  if (hasReceiptFile) {
    const fileName = message.document?.file_name || "Фото / квитанція";
    const caption = message.caption || "";
    const receiptText =
`📎 Квитанція / файл отримано
...
${caption || "Без коментаря"}`;
    ...
    await copyMessage(
      ...
      userChatId,
      message.message_id,
      `📎 Квитанція від клієнта ТихоПоруч\n\n👤 Імʼя: ${firstName}\n🔗 Username: ${username}\n🆔 Chat ID: ${userChatId}\n\n${caption || ""}`
    );
    ...
    await sendMessage(
      userChatId,
      "Дякуємо 💜\n\nКвитанцію отримано. Ми перевіримо оплату та повідомимо вас у цьому чаті."
    );
  ...
  await saveClient({
    name:firstName,
    contact:username,
    tariff:"Повідомлення в боті",
    request:text || "[не текстове повідомлення]",
    note:"",
    source:"telegram",
    status:"new",
    telegram_chat_id:String(userChatId)
